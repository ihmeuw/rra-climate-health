...
def get_CHELSA_projection_old(location_iso3, threshold, scenario, year):
    CLIMATE_ROOT = Path(
        '/mnt/team/rapidresponse/pub/population/data/02-processed-data/human-niche/chelsa-downscaled-projections')
    OVER30_FSTR = "days_above_threshold_{threshold}_{scenario}.nc"
    projection_filepath = CLIMATE_ROOT / location_iso3 / OVER30_FSTR.format(
...
def get_CHELSA_projection_raster_old(location_iso3, threshold, scenario, year,
                                     shapefile, reference_raster, untreated=False):
    NODATA_CLIMATE = -99
...
def get_worldpop_population_raster(location_iso3, return_year=False):
    import glob
    OTHER_GRIDDED_PROJECTS_ROOTPATH = Path(
        '/mnt/team/rapidresponse/pub/population/data/02-processed-data/other-gridded-pop-projects/')  # worldpop-bespoke/NGA/2019.tif')
    OTHER_GRIDDED_PROJECTS_RAW_ROOTPATH = Path(
        '/mnt/team/rapidresponse/pub/population/data/01-raw-data/other-gridded-pop-projects/')  # worldpop-bespoke/NGA/2019.tif')
    ...
    file_paths = glob.glob(
        str(OTHER_GRIDDED_PROJECTS_ROOTPATH / 'worldpop-constrained' / location_iso3 / f"*.tif"),
        recursive=True)
    if len(file_paths) == 0:
        file_paths = glob.glob(
            str(OTHER_GRIDDED_PROJECTS_RAW_ROOTPATH / 'worldpop-constrained' / location_iso3 / f"*.tif"),
            recursive=True)
        ...
        logging.info(
            f"Using WorldPop {worldpop_product} year {year_version} for {location_iso3}")
...
def get_fhs_lsae_location_mapping(short=False, extra_checks=False):
    ...
    lsae_loc_most_detailed = lsae_loc_meta.query("most_detailed == 1").copy()[
        ['location_id', 'ihme_loc_id', 'location_name', 'path_to_top_parent']].rename(
        columns={'location_id': 'lsae_location_id', 'ihme_loc_id': 'lsae_ihme_loc_id',
    ...
    SNNP_lsae_locid = lsae_loc_meta.query(
        "location_name == 'Southern Nations, Nationalities, and Peoples'").location_id.item()
    SNNP_fhs_locid = fhs_loc_meta.query(
        "location_name == 'Southern Nations, Nationalities, and Peoples'").location_id.item()
    fhs_loc_meta.loc[
        fhs_loc_meta.location_id == SNNP_fhs_locid, 'location_id'] = SNNP_lsae_locid
    fhs_loc_most_detailed = fhs_loc_meta.query("most_detailed == 1").copy()[
        ['location_id', 'ihme_loc_id', 'local_id', 'location_name']].rename(
        columns={'location_id': 'fhs_location_id', 'ihme_loc_id': 'fhs_ihme_loc_id',
                 'local_id': 'fhs_local_id', 'location_name': 'fhs_location_name'})
    ...
    exploded_lsae_loc = exploded_lsae_loc.merge(
        lsae_loc_meta[['location_id', 'location_name']].rename(
            columns={'location_id': 'hierarchy',
                     'location_name': 'lsae_hierarchy_name'}), on='hierarchy',
        how='left')
    lsae_fhs_mapping = exploded_lsae_loc.merge(fhs_loc_most_detailed,
                                               left_on='hierarchy',
                                               right_on='fhs_location_id', how='right')
    fhs_loc_meta.loc[
        fhs_loc_meta.location_id == SNNP_lsae_locid, 'location_id'] = SNNP_fhs_locid
    fhs_loc_most_detailed.loc[
        fhs_loc_most_detailed.fhs_location_id == SNNP_lsae_locid, 'fhs_location_id'] = SNNP_fhs_locid

    lsae_fhs_mapping.loc[
        lsae_fhs_mapping.fhs_location_id == SNNP_lsae_locid, 'fhs_location_id'] = SNNP_fhs_locid
    ...
    assert set(fhs_loc_most_detailed.fhs_location_id.unique()) == set(
    ...
    if extra_checks:
        ...
        assert len(set(fhs_loc_most_detailed.fhs_location_id.unique()) - set(
            gbd_shapes.loc_id.unique())) == 0

    return lsae_fhs_mapping if not short else lsae_fhs_mapping[

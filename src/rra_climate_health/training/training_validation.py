...
def validate_model(
    ...
    for fold, (train_df, test_loc, test_year) in enumerate(
        ...
        if model_type == ModelType.LINEAR_MIXED_EFFECTS:
            ...
            for predictor in model_spec.predictors:
                if (
                    ...
                    knots = get_knot_values(
                        df, predictor.name, predictor.spline, var_info
                    )
        ...
        evaluation_sets = [("unseen_locations", test_loc), ("unseen_years", test_year)]
        ...
        for label, test_set in evaluation_sets:
            ...
            if model_type == ModelType.LINEAR_MIXED_EFFECTS:
                ...
                if label == "unseen_locations":
                    with localconverter(default_converter + pandas2ri.converter):
                        r_test_set = pandas2ri.py2rpy(test_set)
                    y_pred = np.array(
                        scam_lib.predict_scam(
                            model,
                            newdata=r_test_set,
                            type="response",
                    ...
                    test_set["ihme_loc_id"] = pd.Categorical(
                        test_set["ihme_loc_id"], categories=train_levels
                    )
                    with localconverter(default_converter + pandas2ri.converter):
                        r_test_set = pandas2ri.py2rpy(test_set)
                    y_pred = np.array(
                        scam_lib.predict_scam(
                            model, newdata=r_test_set, type="response"
                        )
                    )
            ...
            assert (
                not temp_test["predicted_probs"].isnull().any()
            ), "Predicted probabilities contain NaN values."
            test_sets.append(temp_test)
        fold_test_set = pd.concat(test_sets, ignore_index=True)
        # Point-wise metrics
        m = calculate_metrics(
            fold_test_set[target_measure], fold_test_set["predicted_probs"]
        )
        ...
        group_stats = fold_test_set.groupby([location_variable, year_variable]).agg(
            {target_measure: "mean", "predicted_probs": "mean"}
        )

        m["prev_mae"] = mean_absolute_error(
            group_stats[target_measure], group_stats["predicted_probs"]
        )
        m["prev_rmse"] = np.sqrt(
            mean_squared_error(
                group_stats[target_measure], group_stats["predicted_probs"]
    ...
    summary["gbd_rmse"] = np.sqrt(
        mean_squared_error(merged_gbd_data["gbd"], merged_gbd_data["pred"])
    )
    return summary
...
def get_knot_values(
    df: pd.DataFrame,
    variable: str,
    spline: SplineSpecification,
    var_info: dict | None = None,
) -> np.ndarray:
    k = spline.k
    ...
    if knot_strategy == "quantiles":
        ...
        knot_values = np.linspace(
            df[variable].min(), df[variable].max(), inner_knot_n + 2
        )[1:-1]
        ...
        if inner_knot_n not in harrell_knot_values:
            raise ValueError(
                f"Harrell knot specification not defined for {inner_knot_n} knots"
            )
        ...
        knot_values = var_info[variable]["transformer"](
            np.array([spline.knots])
        ).flatten()
    else:

...
# @profile
def get_transformed_variable_raster(
    predictor,
    cm_data,
    cmip6_scenario,
    year,
    ...
    print(
        f"Getting raster for {predictor.name} {year} {cmip6_scenario} {draw} {apply_coefficient}"
    )
    ...
    beta = (
        coefficients.loc[predictor.name].astype(np.float32)
...
def get_model_prevalence(  # noqa: C901 PLR0912
    ...
    coefs, ranefs = cm_data.load_submodel_coefficients(
        spec.version.model, [("age_group_id", age_group_id), ("sex_id", sex_id)]
    )
    ...
    if not threshold_flag_varname:
        ...
        beta_ldi = (
            (
                beta_interaction
                * get_transformed_variable_raster(
                    threshold_predictor,
                    cm_data,
                    cmip6_scenario,
                    year,
                    ...
                    var_info,
                    draw,
                )  # partial_estimates[threshold_flag_varname]
                + coefs.loc["ldi_pc_pd"]
            )
            .to_numpy()
            .astype(np.float32)
    ...
    ldi_version = next(
        (x for x in spec.predictors if x.name == "ldi_pc_pd"), None
    ).version
    prevalence = 0
    for i in range(1, 11):
        gc.collect()
...
def get_ldi_betas(
    ...
    while True:
        ldi_pred_name = (
            "ldi_pc_pd" if current_degree == 1 else f"I(ldi_pc_pd^{current_degree})"
        )
        if ldi_pred_name in coefs.index:
            beta_dict[current_degree] = coefs.loc[ldi_pred_name].astype(np.float32)
            interacting_terms = [
                x for x in coefs.index if ldi_pred_name in x and x != ldi_pred_name
            ]
            for interacting_term in interacting_terms:
                interacting_predictor = [
                    x
                    for x in predictors
                    if x.name == interacting_term.replace(f"{ldi_pred_name}:", "")
                ][0]
                beta_dict[current_degree] += coefs.loc[
                    interacting_term
                ] * get_transformed_variable_raster(
                    interacting_predictor,
                    cm_data,
...
def get_ldi_z_component(
    ...
    z_ldi = rt.RasterArray(
        ...
        * np.power(
            np.array(
                transform_func(
                    cm_data.load_ldi_raster(ldi_scenario, year, dec_str, ldi_version)
                ).astype(np.float32)
            ),
            degree,
        ),
...
def model_inference_main(
    ...
    # will show how coefs need to be formatted:
    ...
    cm_data.save_results_table(
        df, results_version, cmip6_scenario, year, sex_id, age_group_id, draw
    )
...
def forecast_scenarios(
    ...
    for scenario in scenarios:
        ...
        scenario_years = [
            yr
            for yr in years
            if yr >= FIRST_FORECAST_YEAR or scenario == REFERENCE_SCENARIO
        ]
        for year in years:
        ...
        scenario_df.to_parquet(results_path / f"{scenario}.parquet", index=True)
    ...
    if REFERENCE_SCENARIO in scenarios:
        reference = (
            combined.query("scenario == @REFERENCE_SCENARIO")
            .droplevel("scenario")
            .rename(columns={"prevalence": "ref_prev", "affected": "ref_affected"})
        )
...
def model_inference(
    ...
    results_version = cm_data.new_results_version(
        model_version, age_group_id, sex_id, year, cmip6_scenario, draws
    )

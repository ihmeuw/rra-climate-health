...
def get_climate_vars_for_prev_months(
    ...
    lookup_year_months_df = pd.DataFrame(
        lookup_year_months, columns=["year", "month", "suffix"]
    )
    lookup_year_months_df = lookup_year_months_df.merge(
        temp_df[[lat_col, long_col]], how="cross"
    )

    climate_prev_months = pd.merge(
        lookup_year_months_df,
        climate_vars_df,
        on=["year", "month", lat_col, long_col],
        how="inner",
    )
    # concat climate_vars with suffix var and pivot_wide
    climate_prev_months["climate_var_time"] = (
        climate_prev_months["climate_var"] + climate_prev_months["suffix"]
    )
    climate_prev_months.drop(columns=["climate_var", "suffix"], inplace=True)
    climate_prev_months_wide = climate_prev_months.pivot_table(
        index=["lat", "long"],
        columns="climate_var_time",
        values="value",
    ).reset_index()

    # merge with temp_df
    temp_df = temp_df.merge(climate_prev_months_wide, on=["lat", "long"], how="left")
    return temp_df

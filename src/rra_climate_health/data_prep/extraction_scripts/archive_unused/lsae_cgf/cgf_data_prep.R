...
## child_height should be in centimeters, and child_weight should be in kg.
...
##########################################################################################################################################################################################
  ...
  extract_date <- "2019_10_29" # The date that you ran the post_extraction script
  previous_date <- "2018_12_04" # If you want to run coverage plots of only new data,the plots will plot all data incorporated since this date
  cores <- '5'
  cov_plot <- FALSE # Assign TRUE if you want to run coverage plots, assign FALSE if you just want to collapse!
  ...
  work_dir <- paste0("/share/code/geospatial/",Sys.info()['user'],"/cgf/data_prep")
  ...
  user<- Sys.info()['user']    
  package_list <- c('survey','foreach', 'snow', 'haven', 'gdata', 'readstata13')
  ...
  input_data_csv<-paste0(l,"LIMITED_USE/LU_GEOSPATIAL/geo_matched/cgf/test/", extract_date, ".csv")
...
#######################################################################################################################################################
  ...
  # Load in UbCov data
  data<-fread(input_data_csv)# Reading this in as a .csv instead of a .rda or .rdata keeps data the NA values as NA instead of NaN
  data_raw<-copy(data)  
...
########################################################################################################################################################
  ...
  DropData<-function(data=data,indicator){
    ...
    if(indicator %in% c("HAZ","WHZ","CIAF","BMI", "BMIZ")){ # WAZ (no height) excluded from this list of indicators; 2 nids have zscores but no anthro
      data[is.na(child_height) & !(nid %in% c(23017, 319322)), drop:=1]
      data[child_height<=0, drop:=1]
      data[child_height>=180, drop:=1]# I'm going to assert that there are no 6-foot tall 5 year olds.
    }
    ...
    if(indicator %in% c("WAZ","WHZ","CIAF","BMI", "BMIZ")){ # HAZ (no weight) excluded from this list of indicators; 2 nids have zscores but no anthro
      data[is.na(child_weight) & !(nid %in% c(23017, 319322)), drop:=1]
      data[child_weight<=0, drop:=1]
      data[child_weight>=45, drop:=1] # I'm also going to assert that there are no 40 kg (nearly 100 lb) kids 5 years or younger. For context, a 5-year old child of 40 lb is normal
    }
...
##############################################################################################################################################################
  ...
  #data$child_weight <- as.numeric(data$child_weight)
  data$child_height <- as.numeric(data$child_height)
  #data$sex <- as.numeric(data$sex)    
  data[,BMI:= child_weight/((child_height/100)^2)]
  ...
  # Calculate HAZ score
  data[,HAZ:=(((child_height/m) ^ l)-1)/(s*l)]
  ...
  # Calculate WAZ score
  data[,WAZ:=(((child_weight/m) ^ l)-1)/(s*l)]
  data<-data[,!c("l","m","s"),with=F]

  # 4. WHZ (Wasting/WHO-Overweight)

  # Round length to the nearest half-centimeter:
  data[,child_height_rounded := plyr::round_any(child_height, .5)]

  # Merge on WHO data
  data<-merge(data,
              WHZ_mo[,list(l,m,s,sex,child_height_rounded=length,age_cat_1)],
              by=c("sex","age_cat_1","child_height_rounded"),
              all.x = TRUE, allow.cartesian = TRUE)

  # calculate WHZ score
  data[,WHZ :=(((child_weight/m) ^ l)-1)/(s*l)]
  data<-data[,!c("l","m","s"),with=F]
...
#####################################################################################################################################################################
  ...
  #write.csv(data_gbd, paste0(l, "/LIMITED_USE/LU_GEOSPATIAL/geo_matched/cgf/pre_collapse/cgf_lbw_", module_date, ".csv"))

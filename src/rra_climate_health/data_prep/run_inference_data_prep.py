...
def get_climate_vars_all_locs(
    ...
    Concatenates results along climate variable dimension.
    """
...
def get_all_climate_vars_year_months_for_latlongs(
    ...
    """
    Wrapper function to get all monthly climate variables, parellized by year.
    Keeps results in xarray format for speed.
...
def run_neonatal_update(
    ...
    # Use last-updated df which already contains wealth data, corrected ages, etc.
    ...
    # manually assign output root
    output_root += "/neonatal_mortality/training_data/2025_11_20.01/neonatal"
    ...
    # save climate_vars_da as intermediate step data
    ...
    # convert final result to data frame (big, but manageable)
    climate_vars_df = climate_vars_da.to_dataframe().reset_index()
    # save climate_vars_df as intermediate step data
    ...
    # format results data in a way that can be merged back to df_min_age
    ...
    # concatenate results in long format (expanded by birth_month/birth_year,
    # which is what will be used for merging.
    df_results = []
    ...
    # save out long format as intermediate step data
    ...
    # save out wide format as intermediate step data

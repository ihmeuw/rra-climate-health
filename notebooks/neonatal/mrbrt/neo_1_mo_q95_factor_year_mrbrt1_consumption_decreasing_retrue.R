...
## set parameters
summary_file <- paste0("nnm_1_mo_q95_mrbrt_factor_yr_mrbrt1_consumption_decreasing_retrue")
...
# compute mortality rate by grouping by cluster ID. Get standard error. E.g.
# transforming from linear to logit
df_model$se_logit <- mapply(FUN = function(mu, sigma) {
  msm::deltamethod(g = ~log(x1/(1-x1)), mean = mu, cov = sigma^2)
}, mu = df_model$child_mortality, sigma = df_model$se1)


# 1. transform using logit function 0 to 1 to -inf to inf
# 2. run model as below
# 3. take inverse logit to transform results back to 0 and 1?

# will need to do standard errors 
library(msm)
...
get_spline_posteriors(model)

stdevs_tmp <- apply(X = mrtoolr::core$other_sampling$sample_simple_lme_beta(
  sample_size = 1000L, model = model
), MARGIN = 2, FUN = sd)


# get random effects
re_df <- data.frame(level = names(model$re_soln), re = unlist(model$re_soln))
...
colnames(re_df)[2] <- "random_effects"
setorder(re_df,random_effects)

fixed_effects <- data.frame(unlist(model$summary()[[1]]))
colnames(fixed_effects)[1] <- "estimate"

# # Save model summary and random effects to text file
summary_file_path <- paste0(model_summary_dir, summary_file, ".txt")
fe_output <- capture.output(print(fixed_effects, row.names = TRUE))
cat(paste(fe_output, collapse = "\n"), file = summary_file_path)

...
model1 = pd.read_parquet(
    RESULTS_PATH + "predictions_nnm_015_1_mo_model_summary.parquet"
)
model3 = pd.read_parquet(
    RESULTS_PATH + "predictions_nnm_015_3_mo_model_summary.parquet"
)
model6 = pd.read_parquet(
    RESULTS_PATH + "predictions_nnm_015_6_mo_model_summary.parquet"
)
model9 = pd.read_parquet(
    RESULTS_PATH + "predictions_nnm_015_9_mo_model_summary.parquet"
)
...
pdf_path = os.path.join(PLOT_PATH, "neonatal_15pc_time_comparisons_rebinned.pdf")
...
# Troubleshooting binning issues ###############################################

for model, m_name in zip(
    [model1, model3, model6, model9], ["1-month", "3-month", "6-month", "9-month"]
):
    print(f"{m_name}")
    print(model[bin_col_dict[m_name]].nunique())

# for some reason there is only 1 value for days_over_30C_prev_6_mo_avg in model6
################################################################################
...
# Create a PDF to save the plots
pdf_path = os.path.join(PLOT_PATH, "neonatal_15pc_data_points.pdf")

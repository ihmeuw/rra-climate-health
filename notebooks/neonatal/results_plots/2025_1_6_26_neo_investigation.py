...
# 2. days over 30 with 5-year cutoff ####################################################

os.listdir(RESULTS_PATH)
model1 = pd.read_parquet(
    RESULTS_PATH + "predictions_nnm_1_mo_do30_5yr_cutoff_summary.parquet"
)
...
model1 = pd.read_parquet(
    RESULTS_PATH + "predictions_nnm_1_mo_q95_10yr_cutoff_summary.parquet"
)
...
# Create a PDF to save the plots
pdf_path = os.path.join(PLOT_PATH, "neonatal_q95_10_yr_cutoff.pdf")
with PdfPages(pdf_path) as pdf:
    # Create a figure with 4 rows and 3 columns
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(15, 5), constrained_layout=True)
...
# make table of summaries
coef_file_name = "neonatal_q95_10_yr_cutoff_coefs.csv"
...
summaries = [f for f in os.listdir(SUMMARY_DIR) if f.endswith(".txt")]
# only look at q9
summaries = [f for f in summaries if "_q95_10yr_cutoff_summary" in f]
print(summaries)
...
os.listdir(RESULTS_PATH)
model1 = pd.read_parquet(
    RESULTS_PATH + "predictions_nnm_1_mo_q95_mrbrt_factor_yr_mrbrt1.parquet"
)
...
# Create a PDF to save the plots
pdf_path = os.path.join(PLOT_PATH, "neonatal_q95_splines_v1.pdf")
with PdfPages(pdf_path) as pdf:
    # Create a figure with 4 rows and 3 columns
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(15, 5), constrained_layout=True)
...
model1 = pd.read_parquet(
    RESULTS_PATH
    + "predictions_nnm_1_mo_q95_mrbrt_factor_yr_mrbrt1_consumption_decreasing.parquet"
)
...
# Create a PDF to save the plots
pdf_path = os.path.join(PLOT_PATH, "neonatal_q95_splines_v2.pdf")
with PdfPages(pdf_path) as pdf:
    # Create a figure with 4 rows and 3 columns
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(15, 5), constrained_layout=True)
...
model1 = pd.read_parquet(
    RESULTS_PATH
    + "predictions_nnm_1_mo_q95_mrbrt_factor_yr_mrbrt1_consumption_decreasing_retrue.parquet"
)
...
# Create a PDF to save the plots
pdf_path = os.path.join(PLOT_PATH, "neonatal_q95_splines_v3.pdf")
with PdfPages(pdf_path) as pdf:
    # Create a figure with 4 rows and 3 columns
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(15, 5), constrained_layout=True)
...
# Make bar plot of individuals by age_month_original ##########################
...
raw_data = raw_data.dropna(
    subset=["age_month_original", "days_over_30C_prev_0_mo", "q95_prev_0_mo"]
)
...
plt.bar(
    raw_data["age_month_original"].value_counts().sort_index().index,
    raw_data["age_month_original"].value_counts().sort_index().values,
    color="skyblue",
)
plt.axvline(x=60, color="red", linestyle="--", linewidth=1.5)  # Add vertical line at 60
plt.xlabel("Age Month Original", fontsize=14)
plt.ylabel("Number of Individuals", fontsize=14)
plt.title(
    "Distribution of Individuals by Age Month Original\n(Equal to Interview Date - Birth Date)",
    fontsize=16,
)

# Set x-axis ticks to increments of 12
plt.xticks(
    ticks=np.arange(0, raw_data["age_month_original"].max() + 1, 24),  # Increment by 12
    fontsize=10,
    rotation=45,
)

plt.tight_layout()
plt.savefig(PLOT_PATH + "age_month_original_distribution.png")
plt.close()

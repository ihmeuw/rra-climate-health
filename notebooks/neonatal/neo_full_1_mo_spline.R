...
# install.packages('gamm4',lib = "/homes/elyeb/rlibs")
library(gamm4,lib.loc = "/homes/elyeb/rlibs")
# library(mgcv)
# install.packages("mgcv", lib = "/homes/elyeb/rlibs")
library(mgcv,lib.loc = "/homes/elyeb/rlibs")
...
## set parameters
summary_file <- paste0("nnm_full_1_mo_spline_summary")
...
# )
model <- gam(
  child_mortality ~ s(consumption_pd, bs = "mpi") + 
    s(days_over_30C_prev_0_mo, bs = "mpi") +
    total_precipitation_prev_0_mo +
    sex_id +
    birth_year +
    s(ihme_loc_id, bs = "re"),  # Random effect for ihme_loc_id
  data = df_model,
  family = binomial
)

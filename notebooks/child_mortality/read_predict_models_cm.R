...
plot_dir <- "/mnt/team/rapidresponse/pub/population/modeling/climate_malnutrition/child_mortality/plots/2026_02_27.01/"
...
ggsave(
  filename = "mgcv_full_data_consumption_pd.png", 
  plot = p,
  device = "png",
  path = paste0(plot_dir,summary_file,"/"), 
  width = 10, 
...
ggsave(
  filename = "mgcv_full_data_days_over_30C.png", 
  plot = p,
  device = "png",
  path = paste0(plot_dir,summary_file,"/"), 
  width = 10, 
...
# 3/19/2026
summary_file <- "cm_mgcv"
model = readRDS(file = paste0(results_dir, summary_file,".rds"))
plot(model, residuals = FALSE, pch = 1, cex = 1)
...
ggsave(
  filename = "mgcv_partial_data_consumption_pd.png", 
  plot = p,
  device = "png",
  path = paste0(plot_dir,summary_file,"/"), 
  width = 10, 
...
ggsave(
  filename = "mgcv_partial_data_days_over_30C.png", 
  plot = p,
  device = "png",
  path = paste0(plot_dir,summary_file,"/"), 
  width = 10, 
...
# 3/3/2026 - predict version of model that includes latest data updates
...
# 
# # Merge other coefficients on data for manual prediction
# 
# # sex_id
# sex_id_coef_dt <- data.table(sex_id=c("Male","Female"),sex_id_effect=c(0,coefs['sex_idFemale']))
# df_model <- merge(df_model, sex_id_coef_dt, by = "sex_id", all.x = TRUE)
# 
# # birth_year
# birth_year_coef_dt <- data.table(variable=names(coefs),effect=coefs)
# birth_year_coef_dt[,birth_year:=sub("birth_year","",variable)]
# birth_year_coef_dt[,birth_year:= as.numeric(birth_year)]
# birth_year_coef_dt <- birth_year_coef_dt[!is.na(birth_year)]
# birth_year_coef_dt[,birth_year:= as.character(birth_year)]
# # find base year and set to 0
# setdiff(df_model$birth_year,birth_year_coef_dt$birth_year) #1953
# birth_year_coef_dt$variable <- NULL
# birth_year_coef_dt <- rbind(data.table(effect=0,birth_year='1953'),birth_year_coef_dt)
# setnames(birth_year_coef_dt,old="effect",new="birth_year_effect")
# df_model <- merge(df_model, birth_year_coef_dt, by = "birth_year", all.x = TRUE)
# 
...
# 
# # birth year is somewhat complicated as a factor variable:
# birth_year_numeric <- as.numeric(as.character(df_model$birth_year))
# mean_birth_year <- mean(birth_year_numeric)
# closest_birth_year <- df_model$birth_year[which.min(abs(birth_year_numeric - mean_birth_year))]
# mean_birth_year_factored <- factor(closest_birth_year, levels = levels(df_model$birth_year))
# df_model[, mean_birth_year := mean_birth_year_factored]
# 
# # Note which birth year is the mean and adjust below formula accordingly:
# print(mean_birth_year_factored) # 2000
# beta_birth_year <- coefs["birth_year2000"]
# 
# # Calculate linear predictor (fixed effects only)
# # One version at mean values
# df_model$linear_pred_mean <- (
...
#     beta_birth_year # Using coefficient only. 
# )
# # One version variable values -TBC
# df_model$linear_pred_at_data <- (
#   beta_consumption * df_model$consumption_pd +
#     beta_days_over_30C * df_model$days_over_30C +
#     beta_total_precipitation * df_model$total_precipitation + 
#     df_model$sex_id_effect+
#     df_model$birth_year_effect 
# )
# 
...
#   exp(df_model$linear_pred_at_data)
# 
# 
...
# 
# # sex_id
# sex_id_coef_dt <- data.table(sex_id=c("1","2"),sex_id_effect=c(0,coefs['sex_idFemale']))
# df_model <- merge(df_model, sex_id_coef_dt, by = "sex_id", all.x = TRUE)
# 
# 
# # create mean variables
...
# # note that data is coded as 1=male, 2=female
# df_model[,mean_fraction_female:= mean(as.numeric(df_model$sex_id))-1]
# df_model[,mean_birth_year := as.integer(mean(df_model$birth_year))]
# 
# # Calculate linear predictor (fixed effects only)
# # One version at mean values
# df_model$linear_pred_mean <- (
...
# 
# # save results
# write_parquet(df_model,paste0(results_dir,"predictions_",summary_file,"_repredicted.parquet"))
# print(paste0("child mortality predictions saved to ",paste0(results_dir,"predictions_",summary_file,"_repredicted.parquet")))

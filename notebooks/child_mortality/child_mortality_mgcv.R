...
cols <- c("indv_id","child_mortality", "age_month","age_year_at_year_end", "sex_id", "ihme_loc_id", "consumption","consumption_pd","birth_year","int_birth_year_diff_months", climate_vars)
df_model <- df[, ..cols]
...
# fit model
# model_gam <- mgcv::gam(age_year_at_year_end ~ 
#                    s(consumption_pd) +      
#                    s(days_over_30C) +       
#                    total_precipitation + 
#                    sex_id + 
...
model_bam <- mgcv::bam(age_year_at_year_end ~ 
                         s(consumption_pd, k = 3) +      
                         s(days_over_30C, k = 3) +       
                         total_precipitation + 
                         sex_id + 
                         birth_year + 

...
df_model_fe["model_predictions_scaled"] = (
    df_model_fe["model_predictions"] / df_model_fe["age_month"]
)
df_model_me["model_predictions_scaled"] = (
    df_model_me["model_predictions"] / df_model_me["age_month"]
)

max_age = 60
df["months_possible"] = np.nan
df.loc[df["child_mortality"] == 0, "months_possible"] = df.apply(
    lambda x: min(x["age_month"], max_age), axis=1
)
df.loc[df["child_mortality"] == 1, "months_possible"] = max_age
df["months_possible"] = df["months_possible"].astype(int)
df["months_possible"] = df["months_possible"] / 60
df["mortality_per_months_possible"] = df["child_mortality"] / df["months_possible"]

# adjust modeled data
df_model_me["months_possible"] = np.nan
df_model_me.loc[df_model_me["child_mortality"] == 0, "months_possible"] = (
    df_model_me.apply(lambda x: min(x["age_month"], max_age), axis=1)
)
df_model_me.loc[df_model_me["child_mortality"] == 1, "months_possible"] = max_age
df_model_me["months_possible"] = df_model_me["months_possible"].astype(int)
df_model_me["months_possible"] = df_model_me["months_possible"] / 60
df_model_me["mortality_per_months_possible"] = (
    df_model_me["model_predictions"] / df_model_me["months_possible"]
)
# adjust modeled data
df_model_fe["months_possible"] = np.nan
df_model_fe.loc[df_model_fe["child_mortality"] == 0, "months_possible"] = (
    df_model_fe.apply(lambda x: min(x["age_month"], max_age), axis=1)
)
df_model_fe.loc[df_model_fe["child_mortality"] == 1, "months_possible"] = max_age
df_model_fe["months_possible"] = df_model_fe["months_possible"].astype(int)
df_model_fe["months_possible"] = df_model_fe["months_possible"] / 60
df_model_fe["mortality_per_months_possible"] = (
    df_model_fe["model_predictions"] / df_model_fe["months_possible"]
)

# just use df_model
df_model["months_possible"] = np.nan
df_model.loc[df_model["child_mortality"] == 0, "months_possible"] = df_model.apply(
    lambda x: min(x["age_month"], max_age), axis=1
)
df_model.loc[df_model["child_mortality"] == 1, "months_possible"] = max_age
df_model["months_possible"] = df_model["months_possible"].astype(int)
df_model["months_possible"] = df_model["months_possible"] / 60
df_model["raw_mortality_per_months_possible"] = (
    df_model["child_mortality"] / df_model["months_possible"]
)
df_model["modeled_fe_mortality_per_months_possible"] = (
    df_model["mortality_fe_manual"] / df_model["months_possible"]
)
df_model["modeled_me_mortality_per_months_possible"] = (
    df_model["mortality_me_manual"] / df_model["months_possible"]
)
...
plot_heat_map(
    ...
    outfile="raw_heatmap_child_mortality_10_21_pp",
...
plot_heat_map(
    data=df_model.rename(
        columns={
            "mortality_fe_manual": "model_predictions",
        }
    ),
    outfile="fe_heatmap_child_mortality_10_21_pp",
    title="Modeled Child Mortality without Random Effects (per Person-only)",
    bin_cols=columns_to_bin,
    format=".3f",
    vmin=vmin,
    vmax=vmax,
...
plot_heat_map(
    data=df_model.rename(
        columns={
            "mortality_me_manual": "model_predictions",
        }
    ),
    outfile="me_heatmap_child_mortality_10_21_pp",
    title="Modeled Child Mortality with Random Effects (per Person-only)",
    bin_cols=columns_to_bin,
    format=".3f",
    vmin=vmin,
    vmax=vmax,
...
all_values = []
versions = [
    "raw_mortality_per_months_possible",
    "modeled_fe_mortality_per_months_possible",
    "modeled_me_mortality_per_months_possible",
]
for col in columns_to_bin:
    for version in versions:
        vals = heatmap_df.groupby(["consumption", f"{col}_bin"])[version].mean().values
...
plot_heat_map(
    data=df_model.rename(
        columns={
            "raw_mortality_per_months_possible": "model_predictions",
        }
    ),
    outfile="raw_heatmap_child_mortality_10_21_ppt",
...
plot_heat_map(
    data=df_model.rename(
        columns={
            "modeled_fe_mortality_per_months_possible": "model_predictions",
        }
    ),
    outfile="fe_heatmap_child_mortality_10_21_ppt",
...
plot_heat_map(
    data=df_model.rename(
        columns={
            "modeled_me_mortality_per_months_possible": "model_predictions",
        }
    ),
    outfile="me_heatmap_child_mortality_10_21_ppt",

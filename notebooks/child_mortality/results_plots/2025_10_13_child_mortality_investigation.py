...
# if child died, only count as died if it died in first month (still 0 in aod_months var)
# OTHERWISE: recode as child_mortality = 0
df_min_age.loc[
    (df_min_age["child_mortality"] == 1) & (df_min_age["aod_months"] != "0"),
    "child_mortality",
] = 0
df_min_age["age_month"] = 1
...
plt.tight_layout()
plt.savefig(os.path.join(PLOT_PATH, f"child_mortality_by_year.png"))
plt.close()
...
df_max_age = df.loc[df.groupby("indv_id")["age_year_at_year_end"].idxmax()]
...
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig(os.path.join(PLOT_PATH, f"child_mortality_by_age_month_exploded.png"))
plt.close()

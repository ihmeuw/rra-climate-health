...
## set parameters
summary_file <- "cm_splines_full_no_re_custom_knots" 
...
thresh_knots      <- c(1.5, 5.25, 9.3)    # days_over_30C_monthly
...
# Check for non-mononoticity
# Check monotonicity of consumption_pd: fix days_over_30C to median
df_check_cons <- copy(df_model)
df_check_cons[, `:=`(
  sex_id = factor("Male", levels = c("Male", "Female")),
  total_precipitation_monthly = median(df_model$total_precipitation_monthly, na.rm = TRUE),
  birth_year = median(df_model$birth_year, na.rm = TRUE),
  ihme_loc_id = df_model$ihme_loc_id[1],
  days_over_30C_monthly = median(df_model$days_over_30C_monthly, na.rm = TRUE)  # <-- fix this too
)]
df_check_cons[, pred := predict(model, newdata = df_check_cons, type = "response",
                                exclude = "s(ihme_loc_id)")]
# Now group by consumption_pd — should be monotone decreasing
df_check_cons[, .(pred = mean(pred)), by = consumption_pd][order(consumption_pd)]

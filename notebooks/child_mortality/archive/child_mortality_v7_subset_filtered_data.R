...
## set parameters
sample_percent <- 0.25
summary_file <- "cm_v7_subset_filtered"
...
# get baseline hazard for each observation -1
df_model[,age_month_prev := age_month-1]
df_model$cumhaz_baseline_prev <- sapply(df_model$age_month_prev, function(t) {
  get_cumhaz_baseline(t, baseline_hazard)
})
...
## Repeat the above but for t-1
df_model$cumhaz_me_prev <- df_model$frailty * 
  df_model$cumhaz_baseline_prev * 
  exp(df_model$linear_pred)


# Survival probability = exp(-cumulative hazard)
df_model$survival_me_prev <- exp(-df_model$cumhaz_me_prev)

# Mortality probability = 1 - survival
df_model$mortality_me_prev <- 1 - df_model$survival_me_prev


df_model$cumhaz_fe_prev <- df_model$cumhaz_baseline_prev * 
  exp(df_model$linear_pred)

# Survival probability = exp(-cumulative hazard)
df_model$survival_fe_prev <- exp(-df_model$cumhaz_fe_prev)

# Mortality probability = 1 - survival
df_model$mortality_fe_prev <- 1 - df_model$survival_fe_prev

df_model[,step_cum_me := mortality_me-mortality_me_prev]
df_model[,step_cum_fe := mortality_fe-mortality_fe_prev]
...
test <- df_model[,.(age_month,step_cum_me,mortality_point_me,step_cum_fe,mortality_point_fe,cumhaz_me,cumhaz_me_prev,cumhaz_fe,cumhaz_fe_prev)]

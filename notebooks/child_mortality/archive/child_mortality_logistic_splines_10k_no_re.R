...
## set parameters
summary_file <- "cm_10k_logistic_splines_no_re"
...
# Get data sample ~10k rows
sample_percent <- 10000/nrow(df_model) # 
...
# --- Heatmap ---
p3 <- ggplot(pred_grid, aes(x = days_over_30C, y = consumption_pd, fill = pred_prob)) +
  geom_tile() +
  scale_fill_viridis_c(name = "P(mortality\nbefore 60 mo)") +
  labs(x = "Days over 30\u00B0C", y = "Consumption per day",
       title = "Predicted under-5 mortality (population average)") +
  theme_minimal()
ggsave(paste0(plot_dir, summary_file, "_pred_heatmap.png"), p3,

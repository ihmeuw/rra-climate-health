...
with PdfPages(plot_path) as pdf:
    for location in plot_df['location_id'].unique():
        ...
        handles, labels = [], []
        for value_column, color, linestyle in zip(['GBD Estimate', 'Victor Model Estimate', 'Residual Value',  'FE on SDI, location, age, sex, and interactions'], ['blue', 'green', 'red', 'red', 'red', 'black'], ['-', '-', '-',  ':']):
...
with PdfPages(plot_path) as pdf:
    for location in plot_df['location_id'].unique():
        ...
        handles, labels = [], []
        for value_column, color, linestyle in zip(['GBD Estimate', 'Victor Model Estimate', 'Residual Value', 'FE on HAQI, location, age, and sex + IE on location*haqi and location*age*sex'], ['blue', 'green', 'red', 'red', 'red', 'black'], ['-', '-', '-', ':']):
            handles.append(plt.Line2D([0], [0], color=color, linestyle=linestyle, linewidth=2))
            labels.append(value_column)

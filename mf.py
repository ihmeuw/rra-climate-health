...
def get_climate_variable_dataset(scenario, year, climate_var):
    ...
    filepath = paths.CLIMATE_PROJECTIONS_ROOT / subfolder / climate_variables[climate_var] / (str(year) + ".nc")
    
    projection_ds = xr.open_dataset(filepath)
    projection_ds =projection_ds.loc[dict(year=year)]
    return projection_ds
    
def get_climate_variable_raster(scenario, year, climate_var, shapefile, reference_raster, nodata = np.nan, untreated=False):
